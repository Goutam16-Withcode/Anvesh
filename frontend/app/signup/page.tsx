...
import { Compass, Mail, Lock, User, Eye, EyeOff, CheckCircle2, AlertCircle, ArrowLeft } from 'lucide-react';
...
const loadingStates = [
  { text: 'Generating candidate cryptography keys...' },
  { text: 'Initializing Qdrant candidate profile embedding...' },
  { text: 'Mapping skills to canonical ontology nodes...' },
  { text: 'Retrieving initial 500 candidate position universe...' },
  { text: 'Profile activated! Redirecting to discovery dashboard...' },
];
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsLoading(false);
      setErrorMessage(err.message || 'Registration failed. Email may already be taken.');
    }
  };
      ...
      <div className="min-h-screen bg-slate-50/50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          ...
          <p className="mt-1 text-center text-sm text-slate-500">
            Unlock deterministic skill graphs, recommendations, and What-If simulations
          </p>
        </div>
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-6 shadow-xl sm:rounded-3xl sm:px-10 border border-slate-200/90 space-y-6 relative overflow-hidden">
            ...
            {/* Form with Aceternity Inputs */}
            <form onSubmit={handleSubmit} className="space-y-4">
              <LabelInputContainer>
                <label className="text-xs font-bold text-slate-700">Full Name</label>
                <AceternityInput
                  type="text"
                  placeholder="Goutam Anvesh"
                  value={name}
